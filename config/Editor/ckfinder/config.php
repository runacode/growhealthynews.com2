...
$ConfigFilePath = $BasePath . '/config/data.json';
$ConfigTextData = file_get_contents($ConfigFilePath);
$data = json_decode($ConfigTextData);
$EditMode = false;
if (isset($_REQUEST['overwrite'])) {
    $overwritedata = json_decode(file_get_contents(dirname(__FILE__) . '/Pages/' . $_REQUEST['overwrite']));
    if (isset($_REQUEST['EditMode']) && isset($overwritedata->EditMode)) {

        if (strcmp($_REQUEST['EditMode'], $overwritedata->EditMode) === 0) {
            $EditMode = true;
        }
    }
}
...
$config['authentication'] = function () use($EditMode) {
    return true;
};
...
$config['backends'][] = array(
    ...
    'adapter'      => 'local',
    'baseUrl'      => '/images/',
    'root'         => $BasePath , // Can be used to explicitly set the CKFinder user files directory.
    'chmodFiles'   => 0777,
    'chmodFolders' => 0755,
